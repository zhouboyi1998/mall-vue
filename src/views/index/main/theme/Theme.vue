        ...
        <el-card class="main-container" shadow="hover">
            <el-row :gutter="20" class="table-header">
                <el-col :span="7">
                    ...
                    <el-input
                        clearable
                        v-model="params.theme.basicName"
                        placeholder="请输入基调色名称"
                        @change="initTable"
                        @input="initTable"
                    />
                </el-col>
                <el-col :span="7">
                    ...
                    <el-input
                        clearable
                        v-model="params.theme.hoverName"
                        placeholder="请输入辅助色名称"
                        @change="initTable"
                        @input="initTable"
                    />
                </el-col>
...
const initTable = async () => {
    // 发送请求获取表格数据
    let result = await selectThemePage(params.value)
    // 将返回的数据绑定到表格数据中
...
const changeStatus = async (row) => {
    let theme = { id: row.id, status: row.status }
    let result = await updateTheme(theme)
